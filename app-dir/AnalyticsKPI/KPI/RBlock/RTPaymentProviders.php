...
class RTPaymentProviders extends RBlockRT {
    ...
    function __construct(array $filters=null, $limit=null) {
        ...
        $this->qu1 = "
        SELECT  CONCAT(payment_status, ' - ', payment_provider) as provider FROM (
       ...
       ORDER BY provider"
        ...
        $this->setName("Status - Payment Provider[[Payment provider and status]]");
    }
    ...
    function doExecute() { //echo $this->qu1;
        ...
        $topProviders = array();
        $topProvidersList = array();
        foreach ($result as $key => $value) {
            $topProviders[$value[0]] = utf8_encode($value[0]);
            $topProvidersList[] = $value[0];
        }

        $this->values = $topProviders;
        $this->RTtopProvidersList = join('\',\'', $topProvidersList);
    }
}
