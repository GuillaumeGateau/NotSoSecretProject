...
class PerDayEthnicity extends DBlockPerDay{
    ...
    function __construct(array $filters=null, $limit=null) {
        ...
        $this->qu1 = "
            ...
            LEFT JOIN dimension_time AS dt ON dt.id = users.dates
            LEFT JOIN (SELECT dt.id AS dates, SUM(fup.num_of_users_with_ethnicity) AS filled
            FROM fact_users_profile AS fup
            LEFT JOIN dimension_time AS dt ON dt.id = fup.dimension_registered_time_id
            LEFT JOIN dimension_location AS dl ON dl.id = fup.dimension_location_id
        ...
        $this->setName("%Ethnicity[[%Ethnicity - % of users registering on this day with Ethnicity completed on date of sign-up]]");
        $this->setType("float");
        $this->setASCIIName("%Ethnicity");
        $this->setGraphName("%Ethnicity");
        $this->setGraphVar("ethnicity");
        $this->setGraphLabel("%EthnicityCompleted");
    }
