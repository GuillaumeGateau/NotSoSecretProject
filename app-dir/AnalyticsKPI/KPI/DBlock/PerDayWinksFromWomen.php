...
class PerDayWinksFromWomen extends DBlockPerDay {
    ...
    function __construct(array $filters=null, $limit=null) {
        ...
        SELECT dt.id, SUM(fua.num_of_winks) 
            ...
            LEFT JOIN dimension_location AS dl ON dl.id = fua.dimension_location_id
                WHERE dd.gender = \"FEMALE\" AND dt.id > 20110717 AND dt.id >= ".self::$startDate." AND dt.id <= ".self::$endDate." ".
            $whereFilters.
            "GROUP BY dt.id
            ORDER BY dt.id DESC"
        ;

        $this->setName("Winks from Women[[Winks - Total number of winks sent by women]]");
    }
