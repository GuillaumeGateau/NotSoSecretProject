...
class TopSources extends DBlock {

    private $numOfSources;

    function __construct($numOfSources = 20,$lastDate=null) {
        parent::__construct();

        if($lastDate==null) {
            $lastDate = "CAST(DATE_SUB(CURDATE(),INTERVAL 1 DAY) AS UNSIGNED) ";
        }

        $this->qu1 = "
            SELECT IF(source='(direct)','UNKNOWN',IF(source IS NULL,'UNKNOWN',source)) as src, SUM(num_of_registrations) FROM fact_registrations AS fp
